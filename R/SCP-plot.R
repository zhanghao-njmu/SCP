...
#'
#' library(cowplot)
#' p1 <- CellDimPlot(pancreas_sub, c("Phase", "SubCellType"), label = TRUE) # plot is generated by plot_grid
#' p2 <- FeatureDimPlot(pancreas_sub, "Ins1") + FeatureDimPlot(pancreas_sub, "Ins2") # plot is generated by patchwork
...
#' @importFrom grid grob unit convertWidth convertHeight convertUnit is.unit
#' @importFrom cowplot plot_grid as_gtable
#' @export
#'
panel_fix <- function(x = NULL, panel_index = NULL, respect = NULL,
                      width = NULL, height = NULL, margin = 0.5, space = 0.1, units = "in",
  ...
  for (i in panel_index) {
    panel_index_w <- grob[["layout"]][["l"]][i]
    panel_index_h <- grob[["layout"]][["t"]][i]
    if (is.null(width)) {
    ...
    if (length(geom_index) > 0) {
      ...
      for (j in geom_index) {
        ...
        if (!is.null(subgrob$grobs[[1]][["children"]]) && all(sapply(subgrob$grobs[[1]][["children"]], function(x) inherits(x, "recordedGrob")))) {
          subgrob <- panel_fix_single(subgrob$grobs[[1]][["children"]], width = width, height = height, margin = space, units = units, raster = raster, dpi = dpi, return_grob = TRUE)
        } else {
...
panel_fix_single <- function(x, panel_index = NULL, respect = NULL,
  ...
  if (input_type == "gList") {
    panel_index <- 1
    panel_index_h <- panel_index_w <- nw <- nh <- 1
    w <- h <- unit(1, "null")
  ...
  # print(paste0("raw_aspect:", raw_aspect))
  # print(paste0("width:",width))
  # print(paste0("height:",height))
...
#' @export
#'
CellDimPlot <- function(srt, group.by = "orig.ident", reduction = NULL, dims = c(1, 2), split.by = NULL, cells = NULL,
...
#' FeatureDimPlot(pancreas_sub, features = "G2M_score", reduction = "UMAP", hex = TRUE)
#'
...
#' FeatureDimPlot(pancreas_sub, c("Ins1", "Gcg", "Sst", "Ghrl"), reduction = "UMAP", bg_cutoff = 1, lower_cutoff = 1, upper_cutoff = 4)
...
FeatureDimPlot <- function(srt, features, reduction = NULL, dims = c(1, 2), split.by = NULL, cells = NULL, slot = "data", assay = NULL,
  ...
  if (isTRUE(compare_features) && length(features) > 1) {
    for (s in levels(dat_sp[[split.by]])) {
      ...
      p <- plot_grid(grob)
      plist[[paste0(s, ":", paste0(features, collapse = "|"))]] <- p
    }
  } else {
    for (f in features) {
      for (s in levels(dat_sp[[split.by]])) {
...
#' @importFrom utils askYesNo
#' @export
CellDimPlot3D <- function(srt, group.by = "orig.ident", reduction = NULL, dims = c(1, 2, 3), axis_labs = NULL,
...
#' FeatureStatPlot(pancreas_sub, stat.by = c("Rbp4", "Pyy"), group.by = "SubCellType", comparisons = list(c("Alpha", "Beta"), c("Alpha", "Delta")), symnum.args = list())
#' FeatureStatPlot(pancreas_sub, stat.by = c("Rbp4", "Pyy"), group.by = "SubCellType", bg.by = "CellType", stack = TRUE)
...
#' @importFrom ggplot2 geom_blank geom_violin geom_rect geom_boxplot layer_scales position_jitterdodge position_dodge stat_summary scale_x_discrete element_line element_text element_blank annotate mean_sdl
...
FeatureStatPlot <- function(srt, stat.by, group.by = NULL, split.by = NULL, bg.by = NULL, fill.by = c("group", "feature", "expression"),
                            ...
                            y.min = NULL, y.max = NULL, same.y.lims = FALSE, y.trans = "identity", y.nbreaks = 5,
                            sort = FALSE, stack = FALSE, flip = FALSE,
                            comparisons = list(), ref_group = NULL, pairwise_method = "wilcox.test", symnum.args = list(
                              cutpoints = c(0, 0.0001, 0.001, 0.01, 0.05, 2),
                              symbols = c("****", "***", "**", "*", "ns")
                            ),
                            multiplegroup_comparisons = FALSE, multiple_method = "kruskal.test",
                            theme_use = "theme_scp", aspect.ratio = NULL, title = NULL, subtitle = NULL, xlab = NULL, ylab = "Expression level",
                            legend.position = "right", legend.direction = "vertical",
                            combine = TRUE, nrow = NULL, ncol = NULL, byrow = TRUE, align = "hv", axis = "lr", force = FALSE) {
  ...
  cells <- cells %||% Cells(srt)
  meta.data <- srt@meta.data[cells, , drop = FALSE]
  assay <- assay %||% DefaultAssay(srt)
  exp.data <- GetAssayData(srt, assay = assay, slot = slot)
  ...
  if (is.null(group.by)) {
    ...
    xlab <- ""
    meta.data[[group.by]] <- factor(paste0(stat.by, collapse = ","))
  }
  if (is.null(split.by)) {
  ...
  if (length(features_gene > 0)) {
    dat_gene <- t(exp.data[features_gene, , drop = FALSE])[cells, , drop = FALSE]
  } else {
  ...
  if (isTRUE(same.y.lims) && is.null(y.max)) {
    y.max <- max(as.matrix(dat_exp[, stat.by, drop = FALSE])[is.finite(as.matrix(dat_exp[, stat.by, drop = FALSE]))], na.rm = TRUE)
  }
  ...
  for (g in group.by) {
    ...
    for (f in stat.by) {
      if (isTRUE(stat_single)) {
        ...
        splits <- levels(dat_use[[split.by]])
      } else {
        groups <- list(levels(dat_use[[g]]))
        splits <- list(levels(dat_use[[split.by]]))
      }
      for (single_group in groups) {
        for (sp in splits) {
          ...
          if (fill.by == "group") {
            dat[, "fill.by"] <- if (split.by == "No.split.by") dat[, "group.by"] else dat[, "split.by"]
          ...
          if (isTRUE(multiplegroup_comparisons)) {
            check_R("ggpubr")
            p <- p + ggpubr::stat_compare_means(method = multiple_method, label.y = Inf, vjust = 1.2, hjust = 0, size = 3.5)
...
#' @examples
#' data("pancreas_sub")
#' CellStatPlot(pancreas_sub, stat.by = "Phase", group.by = "CellType", label = TRUE)
#' CellStatPlot(pancreas_sub, stat.by = "Phase", group.by = "CellType", stat_type = "count", position = "dodge", label = TRUE)
#'
...
#' CellStatPlot(pancreas_sub, stat.by = c("Ductal", "Endocrine", "G1S", "G2M"), plot_type = "venn", stat_level = TRUE)
#' CellStatPlot(pancreas_sub, stat.by = c("Ductal", "Endocrine", "G1S", "G2M"), plot_type = "upset", stat_level = TRUE)
#'
#' @importFrom Seurat Cells
#' @export
CellStatPlot <- function(srt, stat.by, group.by = NULL, split.by = NULL, cells = NULL, flip = FALSE,
  ...
  plot <- StatPlot(
    meta.data = meta.data, stat.by = stat.by, group.by = group.by, split.by = split.by, flip = flip,
    ...
    theme_use = theme_use, aspect.ratio = aspect.ratio, title = title, subtitle = subtitle, xlab = xlab, ylab = ylab,
    legend.position = legend.position, legend.direction = legend.direction,
    combine = combine, nrow = nrow, ncol = ncol, byrow = byrow, align = align, axis = axis, force = force
  )
  return(plot)
}
...
#' @importFrom ggplot2 ggplot ggplotGrob aes labs position_identity position_stack position_dodge2 scale_x_continuous scale_y_continuous geom_col geom_area geom_vline scale_fill_manual scale_fill_identity scale_color_identity scale_fill_gradientn guides guide_legend element_line coord_polar annotate geom_sf theme_void after_stat scale_size_area
#' @importFrom ggnewscale new_scale_color new_scale_fill
#' @importFrom ggrepel geom_text_repel
...
#' @importFrom rlang %||%
#' @export
StatPlot <- function(meta.data, stat.by, group.by = NULL, split.by = NULL, flip = FALSE,
                     NA_color = "grey", NA_stat = TRUE, keep_empty = FALSE, stat_single = FALSE, stat_level = NULL,
                     plot_type = c("bar", "rose", "ring", "pie", "trend", "area", "dot", "sankey", "chord", "venn", "upset"),
                     stat_type = c("percent", "count"), position = c("stack", "dodge"),
                     palette = "Paired", palcolor = NULL, alpha = 1,
  ...
  dat_all <- meta.data[, unique(c(stat.by, group.by, split.by)), drop = FALSE]
  ...
  if (plot_type %in% c("bar", "rose", "ring", "pie", "trend", "area", "dot")) {
    ...
    for (g in group.by) {
      for (sp in levels(dat_all[[split.by]])) {
        ...
        if (any(is.na(dat_all[dat_all[[split.by]] == sp, stat.by])) && isTRUE(NA_stat)) {
        ...
        for (single_group in groups) {
          ...
          if (plot_type == "trend") {
            dat_area <- dat[rep(seq_len(nrow(dat)), each = 2), ]
    ...
    for (s in levels(dat_all[[split.by]])) {
      dat_use <- dat_all[dat_all[[split.by]] == s, , drop = FALSE]
      if (plot_type == "venn") {
...
#' @param Class_palette Name of palette to use for CellDimPlot.
#' @param Exp_palette Name of palette to use for FeatureDimPlot.
...
SummaryPlot <- function(srt,
  ...
  if (!is.null(size)) {
    p <- panel_fix_single(p, width = width, height = height, units = units)
  }
  return(p)
}
...
GroupHeatmap <- function(srt, features = NULL, group.by = NULL, split.by = NULL, grouping.var = NULL, numerator = NULL, cells = NULL,
  ...
  if (!is.null(grouping.var)) {
    ...
    if (!is.factor(srt[[grouping.var, drop = TRUE]])) {
      srt[[grouping.var, drop = TRUE]] <- factor(srt[[grouping.var, drop = TRUE]], levels = unique(srt[[grouping.var, drop = TRUE]]))
    }
    ...
    srt[["grouping.var.use", drop = TRUE]] <- srt[[grouping.var, drop = TRUE]] == numerator
    add_dot <- FALSE
  ...
  for (cell_group in group.by) {
    ...
    if (!is.null(split.by)) {
      ...
      levels <- apply(expand.grid(levels(srt[[split.by, drop = TRUE]]), levels(cell_groups[[cell_group]])), 1, function(x) paste0(x[2:1], collapse = " : "))
      cell_groups[[cell_group]] <- setNames(paste0(cell_groups[[cell_group]][cells], " : ", srt[[split.by, drop = TRUE]][cells]), cells)
    ...
    if (!is.null(grouping.var)) {
      ...
      cell_groups[[cell_group]] <- setNames(paste0(cell_groups[[cell_group]][cells], " ; ", srt[["grouping.var.use", drop = TRUE]][cells]), cells)
      cell_groups[[cell_group]] <- factor(cell_groups[[cell_group]], levels = levels[levels %in% cell_groups[[cell_group]]])
    }
  }

  gene <- features[features %in% rownames(srt[[assay]])]
  gene_unique <- features_unique[features %in% rownames(srt[[assay]])]
  meta <- features[features %in% colnames(srt@meta.data)]

  mat_raw <- as.matrix(rbind(GetAssayData(srt, assay = assay, slot = slot)[gene, cells, drop = FALSE], t(srt@meta.data[cells, meta, drop = FALSE])))[features, , drop = FALSE]
  rownames(mat_raw) <- features_unique
  if (isTRUE(lib_normalize) && min(mat_raw, na.rm = TRUE) >= 0) {
    if (!is.null(libsize)) {
      libsize_use <- libsize
    } else {
      libsize_use <- colSums(GetAssayData(srt, slot = "counts", assay = assay)[, colnames(mat_raw), drop = FALSE])
  ...
  for (i in seq_along(group.by)) {
    ...
    if (is.null(split.by)) {
      ...
      column_split_list[[cell_group]] <- factor(gsub(" : .*", "", levels(cell_groups[[cell_group]])), levels = levels(srt[[cell_group, drop = TRUE]]))
    }
    ...
    if (cell_group != "No.group.by") {
      funbody <- paste0(
        "
        grid.rect(gp = gpar(fill = palette_scp(", paste0("c('", paste0(levels(srt[[cell_group, drop = TRUE]]), collapse = "','"), "')"), ",palette = '", group_palette[i], "',palcolor=c(", paste0("'", paste0(group_palcolor[[i]], collapse = "','"), "'"), "))[nm]))
      "
      )
      ...
      lgd[[cell_group]] <- Legend(
        title = cell_group, labels = levels(srt[[cell_group, drop = TRUE]]),
        legend_gp = gpar(fill = palette_scp(levels(srt[[cell_group, drop = TRUE]]), palette = group_palette[i], palcolor = group_palcolor[[i]])), border = TRUE
      )
    }
    ...
    if (!is.null(split.by)) {
      funbody <- paste0(
        "
      grid.rect(gp = gpar(fill = palette_scp(", paste0("c('", paste0(levels(srt[[split.by, drop = TRUE]]), collapse = "','"), "')"), ",palette = '", cell_split_palette, "',palcolor=c(", paste0("'", paste0(unlist(cell_split_palcolor), collapse = "','"), "'"), "))[nm]))
    "
      )
      funbody <- gsub(pattern = "\n", replacement = "", x = funbody)
      eval(parse(text = paste("panel_fun <- function(index, nm) {", funbody, "}", sep = "")), envir = environment())
  ...
  if (!is.null(split.by)) {
    lgd[[split.by]] <- Legend(
      title = split.by, labels = levels(srt[[split.by, drop = TRUE]]),
      legend_gp = gpar(fill = palette_scp(levels(srt[[split.by, drop = TRUE]]), palette = cell_split_palette, palcolor = cell_split_palcolor)), border = TRUE
    )
  }
...
FeatureHeatmap <- function(srt, features = NULL, cells = NULL, group.by = NULL, split.by = NULL, max_cells = 100, cell_order = NULL, border = TRUE, flip = FALSE,
  ...
  for (cell_group in group.by) {
    ...
    if (is.null(split.by)) {
      ...
      cell_groups[[cell_group]] <- unlist(lapply(levels(srt[[cell_group, drop = TRUE]]), function(x) {
        ...
        cells_sub <- intersect(cells, cells_sub)
        cells_tmp <- NULL
        for (sp in levels(srt[[split.by, drop = TRUE]])) {
          cells_sp <- cells_sub[srt[[split.by, drop = TRUE]][cells_sub] == sp]
  ...
  mat_raw <- as.matrix(rbind(GetAssayData(srt, assay = assay, slot = slot)[gene, all_cells, drop = FALSE], t(srt@meta.data[all_cells, meta, drop = FALSE])))[features, , drop = FALSE]
  rownames(mat_raw) <- features_unique
  if (isTRUE(lib_normalize) && min(mat_raw, na.rm = TRUE) >= 0) {
    if (!is.null(libsize)) {
      libsize_use <- libsize
...
DynamicHeatmap <- function(srt, lineages, features = NULL, feature_from = lineages, use_fitted = FALSE, border = TRUE, flip = FALSE,
  ...
  cell_union <- unique(colnames(srt)[apply(srt@meta.data[, lineages, drop = FALSE], 1, function(x) !all(is.na(x)))])
  ...
  if (is.null(features)) {
    ...
    for (l in lineages) {
      ...
      DynamicFeatures <- DynamicFeatures[features, ]
      dynamic[[l]] <- DynamicFeatures
    }
  }
  ...
  feature_metadata <- feature_metadata[rownames(feature_metadata) %in% features, ]
  ...
  if (isTRUE(use_fitted)) {
    ...
    for (l in lineages) {
      ...
      mat_tmp <- as.matrix(rbind(GetAssayData(srt, assay = assay, slot = slot)[gene, cells, drop = FALSE], t(srt@meta.data[cells, meta, drop = FALSE])))[features, , drop = FALSE]
...
#' EnrichmentPlot(pancreas_sub, group_by = "CellType", plot_type = "comparison")
#'
#' EnrichmentPlot(pancreas_sub, group_by = "CellType", group_use = c("Ductal", "Endocrine"), plot_type = "bar", ncol = 1)
...
#' EnrichmentPlot(pancreas_sub, group_by = "CellType", group_use = c("Ductal", "Endocrine"), plot_type = "comparison")
#'
#' pancreas_sub <- RunEnrichment(srt = pancreas_sub, group_by = "CellType", db = c("MP", "DO"), convert_species = TRUE, species = "Mus_musculus")
#' EnrichmentPlot(pancreas_sub, db = c("MP", "DO"), group_by = "CellType", group_use = c("Ductal", "Endocrine"))
#'
#' @importFrom ggplot2 ggplot geom_bar geom_text labs scale_fill_manual scale_y_continuous facet_grid coord_flip scale_color_gradientn scale_fill_gradientn scale_size guides geom_segment expansion guide_colorbar scale_color_manual
...
EnrichmentPlot <- function(srt, db = "GO_BP", group_by = NULL, group_use = NULL, test.use = "wilcox",
                           ...
                           exclude_words = c("cell", "cellular", "dna", "rna", "protein", "development", "organization", "system", "regulation", "positive", "negative", "response", "process"),
                           aspect.ratio = 1, base_size = 12, character_width = 50, lineheight = 0.7,
                           legend.position = "right", legend.direction = "vertical",
                           combine = TRUE, nrow = NULL, ncol = NULL, byrow = TRUE, align = "hv", axis = "lr") {
  plot_type <- match.arg(plot_type)
  ...
  if (plot_type == "comparison") {
    ...
    suppressWarnings(p <- ggplot(res_sub, aes(x = Groups, y = Description)) +
      ...
      scale_size(name = "GeneRatio", range = c(1, 6)) +
      ...
      guides(colour = guide_legend("Non-sig", override.aes = list(colour = "black", fill = "grey80", size = 3))) +
...
#' GSEAPlot(pancreas_sub, group_by = "CellType", group_use = "Ngn3 low EP")
...
#' GSEAPlot(pancreas_sub, group_by = "CellType", plot_type = "comparison")
#' GSEAPlot(pancreas_sub, group_by = "CellType", plot_type = "comparison", pvalueCutoff = 0.05, padjustCutoff = NULL)
#' @importFrom ggplot2 ggplotGrob ggplot aes theme theme_classic alpha element_blank element_rect margin geom_line geom_point geom_rect geom_linerange geom_hline geom_vline geom_segment annotate ggtitle labs xlab ylab scale_x_continuous scale_y_continuous scale_color_manual scale_alpha_manual guides guide_legend
#' @importFrom ggrepel geom_text_repel
...
GSEAPlot <- function(srt, db = "GO_BP", group_by = NULL, group_use = NULL, test.use = "wilcox",
                     ...
                     topTerm = 6, geneSetID = NULL, NES_pos = FALSE, subplots = 1:3, rel_heights = c(1.5, 0.5, 1), rel_width = 3,
  ...
  res <- res[use]
  enrichment <- enrichment[enrichment[["Database"]] %in% db & enrichment[["Groups"]] %in% group_use, ]
  ...
  if (plot_type == "comparison") {
    ids <- NULL
    for (i in group_use) {
      df <- enrichment[enrichment[["Groups"]] == i, ]
      df <- df[df[["pvalue"]] <= pvalueCutoff & df[["p.adjust"]] <= padjustCutoff, ]
    ...
    terms <- setNames(res_sub[["Description"]], res_sub[["ID"]])
    res_sub[["Description"]] <- factor(res_sub[["Description"]], levels = rev(terms[ids]))
    res_sub[["Significant"]] <- res_sub[["pvalue"]] <= pvalueCutoff & res_sub[["p.adjust"]] <= padjustCutoff
