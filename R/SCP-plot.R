...
#' ClassDimPlot(pancreas1k, group.by = "SubCellType", reduction = "UMAP", lineages = paste0("Lineage", 1:3), span = 0.1)
#'
#' pancreas1k <- RunPAGA(srt = pancreas1k, group_by = "SubCellType", liner_reduction = "PCA", nonliner_reduction = "UMAP", return_seurat = TRUE)
#' ClassDimPlot(pancreas1k, group.by = "SubCellType", paga = pancreas1k@misc$paga)
#' ClassDimPlot(pancreas1k,
#'   group.by = "SubCellType", pt.size = 5, pt.alpha = 0.2,
...
#' ClassDimPlot(pancreas1k, group.by = "SubCellType", paga = pancreas1k@misc$paga, paga_show_transition = TRUE)
#' ClassDimPlot(pancreas1k, group.by = "SubCellType", pt.size = 5, pt.alpha = 0.2, velocity = "stochastic")
#' ClassDimPlot(pancreas1k, group.by = "SubCellType", pt.size = 5, pt.alpha = 0.2, velocity = "stochastic", velocity_plot_type = "grid")
#' ClassDimPlot(pancreas1k, group.by = "SubCellType", pt.size = 5, pt.alpha = 0.2, velocity = "stochastic", velocity_plot_type = "grid", velocity_scale = 1.5)
#' ClassDimPlot(pancreas1k, group.by = "SubCellType", pt.size = 5, pt.alpha = 0.2, velocity = "stochastic", velocity_plot_type = "stream")
#' ClassDimPlot(pancreas1k,
#'   group.by = "SubCellType", pt.size = 5, pt.alpha = 0.2,
...
ClassDimPlot <- function(srt, group.by = "orig.ident", reduction = NULL, dims = c(1, 2), split.by = NULL,
                         ...
                         lineages = NULL, lineage_palette = "Dark2", span = 0.75,
                         arrow_reverse = FALSE, line_size = 1, line_bg = "white", line_bg_r = 0.5,
  ...
  for (g in group.by) {
    ...
    for (s in levels(dat_use[[split.by]])) {
      ...
      if (!is.null(lineages)) {
        curve_list <- lapply(lineages, function(l) {
          dat_sub <- dat[!is.na(dat[[l]]), ]
          dat_sub <- dat_sub[order(dat_sub[[l]]), ]
          xlo <- loess(formula(paste("x", l, sep = "~")), data = dat_sub, span = span, degree = 2)
          ylo <- loess(formula(paste("y", l, sep = "~")), data = dat_sub, span = span, degree = 2)
          dat_smooth <- data.frame(x = xlo$fitted, y = ylo$fitted)
          dat_smooth <- dat_smooth[dat_smooth[["x"]] <= max(dat[["x"]]) & dat_smooth[["x"]] >= min(dat[["x"]]), ]
          dat_smooth <- dat_smooth[dat_smooth[["y"]] <= max(dat[["y"]]) & dat_smooth[["y"]] >= min(dat[["y"]]), ]
          dat_smooth[, "Lineages"] <- factor(l, levels = lineages)
          dat_smooth <- unique(na.omit(dat_smooth))
          out <- c(
            geom_path(
              data = dat_smooth, mapping = aes(x = x, y = y), color = line_bg,
              size = line_size + line_bg_r, arrow = arrow(length = unit(0.1, "inches"), ends = ifelse(isTRUE(arrow_reverse), "first", "last")),
              show.legend = TRUE, inherit.aes = FALSE
            ),
            geom_path(
              data = dat_smooth, mapping = aes(x = x, y = y, color = Lineages),
              size = line_size, arrow = arrow(length = unit(0.1, "inches"), ends = ifelse(isTRUE(arrow_reverse), "first", "last")),
              show.legend = TRUE, inherit.aes = FALSE
            )
          )
          return(out)
        })
        curve_list <- unlist(curve_list)
        curve_list <- c(list(new_scale_color()), curve_list, list(scale_color_manual(values = palette_scp(x = lineages, palette = lineage_palette))))
        suppressMessages({
        ...
        p <- suppressWarnings({
          p + curve_list + theme(legend.position = "none")
        })
          ...
          curve_list <- lapply(lineages, function(l) {
            ...
            out <- c(
              ...
              geom_path(
                ...
                size = line_size, arrow = arrow(length = unit(0.1, "inches"), ends = ifelse(isTRUE(arrow_reverse), "first", "last")),
                show.legend = TRUE, inherit.aes = FALSE
              ), scale_color_manual(values = palette_scp(x = lineages, palette = lineage_palette))
            )
            return(out)
          })
          curve_list <- unlist(curve_list)
          curve_list <- c(list(new_scale_color()), curve_list)
          suppressMessages({
            legend_list[["lineages"]] <- get_legend(ggplot() +
              curve_list +
              theme_scp(
...
ClassDimPlot3D <- function(srt, group.by = "orig.ident", reduction = NULL, dims = c(1, 2, 3), axis_labs = NULL,
  ...
  df_legend <- dat_use[which(!duplicated(dat_use[["color"]])), ]
  ...
  for (i in unique(stat.by)) {
    ...
    if (plot_type %in% c("venn", "upset")) {
      if (!is.logical(srt[[i, drop = TRUE]])) {
        if (length(levels) == 2) {
          srt[[i, drop = TRUE]] <- srt[[i, drop = TRUE]] == levels[1]
        } else {
...
#' EnrichmentPlot
#'
#' @param x A enrichemnt result generated by the \link{RunEnrichment} function.
#' @param Enrichment Enrichment column name in \code{x}.
#' @param Groups Groups column name in \code{x}.
...
#' @export
#'
EnrichmentPlot <- function(x, enrichment = "GO_BP", plot_type = "bar", topTerm = 6, topGene = 100,
  ...
  if (length(enrichment) == sum(!enrichment %in% x[["Enrichment"]])) {
...
DynamicHeatmap <- function(srt, lineages, feature_from = lineages,
                           ...
                           n_cluster = NULL, cluster_by = lineages, cluster_method = "mfuzz", fuzzification = NULL, show_fuzzification = FALSE,
...
GSEAPlot <- function(x, geneSetID,
  ...
  if (length(geneSetID) == 1) {
    gsdata <- gsInfo(object = x, geneSetID = geneSetID)
  } else {
    gsdata <- do.call(rbind, lapply(geneSetID, gsInfo, object = x))
  }
  padj <- x[geneSetID, c("Description", "p.adjust", "enrichmentScore")]
  ...
  p1 <- p1 + ylab("Enrichment Score") + theme(
    axis.text.x = element_blank(),
    axis.ticks.x = element_blank(),
    axis.line = element_blank(),
